...
#' @param element name of the element to be extracted. If multiple occurences are found, will extract all
...
eml_empty <- function(x){
  if(is.character(x) && length(x) > 0)
    ...
    all(vapply(names(getSlots(class(x))), function(s) eml_empty(methods::slot(x, s)), logical(1)))
