...
s4_to_xml <- function(obj, root = NULL, ns = eml_namespaces){
  ...
  lapply(fields, function(child){
    ...
    if(is.null(node) || emptynode(node)){             # slot is empty
      ...
      xml2::xml_set_attr(xml, child, as.character(node))
      ...
      xml <- xml2::xml_add_child(xml, child)
      lapply(node[[1]], function(n) xml2::xml_add_child(xml, n))
