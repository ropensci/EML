...
eml_measurementScale <- function(unit.def, col.class){
  ...
  switch(col.class,
         ...
         ratio = new("measurementScale", ratio = new("ratio", unit = eml_unit(unit.def[[1]]))), # FIXME has precision and numericDomain@numberType, numericDomain@boundsGroup
         dateTime = new("measurementScale", dateTime = eml_dateTime(unit.def[[1]]) ), 
         interval = new("measurementScale", interval = new("interval", unit = eml_unit(unit.def[[1]])))
         )
}
...
eml_unit <- function(unit){
  if(is_standard_unit(unit)){ 
    new("unit", standardUnit = unit)
  } else {
    new("unit", customUnit = unit)
...
create_custom_unit <- function(id,
  ...
  assign("custom_units", c(custom_units, unit.def), envir=EMLConfig)
}
...
serialize_custom_units <- function(custom_units){
  ...
  new("additionalMetadata", 
      metadata = new("metadata", unitList))
