...
has_extension <- function(element, ns) {
  xml_find_all(
    element,
    "./*/xs:simpleContent/xs:extension | ./*/xs:complexContent/xs:extension",
    ns = ns
  ) %>%
...
set_class_list <-
  function(class, cf = "classes.R", mf = "methods.R") {
    base <- gsub("^ListOf", "", class)
    write(
      sprintf(
        "setClass('%s', contains = 'list', validity = function(object){ if(!all(sapply(object, is, '%s'))){ 'not all elements are %s objects'; } else { TRUE }})",
        class,
        base,
        base
      ),
...
parse_element <-
  function(node,
    ...
    if (!is.na(name)) {
      ...
      extends <-
        has_extension(node, ns) %>% strip_namespace() %>% replace_character_type()
...
parse_group <-
  function(node, ns, cf = "classes.R", mf = "methods.R") {
    ref <- xml_attr(node, "ref")
    if (is.na(ref))
      create_class(node, ns, cf = cf, mf = mf)
    else {
      slots <-
        ref %>% replace_character_type() %>% strip_namespace() %>% setNames(., .)
      list(
        slots = slots,
        contains = character(),
        slot_order = names(slots)
...
parse_attribute <-
  function(node, ns, cf = "classes.R", mf = "methods.R") {
    ...
    if (is.na(name))
      ...
      list(
        slots = setNames("xml_attribute", strip_namespace(name)),
        contains = character(),
        slot_order = character()
      )
    }
...
create_class <-
  function(node,
    ...
    list(slots = purrr::flatten(purrr::map(s4_bits, `[[`, "slots")),
         contains = unique(as.character(purrr::flatten(
           purrr::map(s4_bits, `[[`, "contains")
         ))))
  }
...
declare_class <-
  function(class,
    ...
    if (length(slots) > 0) {
      ...
      write(
        sprintf(
          "setClass('%s', slots = c(%s), contains = c(%s)) ## A",
          class,
          ## FIXME drop contains = character unless mixed?
          print_cmd(slots),
          print_cmd(contains)
        ),
        "A-classes.R",
      ...
      if (identical(contains, "eml-2.1.1"))
        # hack to avoid no .Data error??
      ...
      write(
        sprintf(
          "setClass('%s', contains = c(%s)) ## %s",
          class,
          print_cmd(contains),
          mark
        ),
        paste0(mark, "-classes.R"),
        append = TRUE
      )
...
get_descriptions <- function(elements) {
  out <-
    elements %>% purrr::map(xml2::xml_find_all,
                            "./xs:annotation/xs:appinfo/doc:summary",
                            ns)
  setNames(out, xml_attr(elements, "name"))
}

add_roxygen <-
  function(slots, slot_descriptions, cf = "A-classes.R") {
...
create_classes <- function(xsd_file,
  ...
  write(c(
    readclass("list-classes.R"),
    readclass("B-classes.R"),
    readclass("A-classes.R"),
    readclass("C-classes.R"),
    readclass("D-classes.R")
  ),
  cf,
  append = append)
  unlink(c(
