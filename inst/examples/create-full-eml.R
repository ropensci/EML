...
R_person <- as.person("Ma?l Ellison <fakeaddress@email.com> [cre]")
...
testthat::test_that("We return TRUE when validating a valid just created eml object with unit list", {
  attributes <-
    data.frame(
      attributeName = c(
        "run.num",
      ...
      attributeDefinition = c(
        ...
        "what variable being measured in what treatment (character/factor).",
        "value of measured variable for run.num on year/day/hour.min.",
        "length of the species in terameters (dummy example of numeric data)"),
      formatString = c(
        NA,
        "YYYY",
        "DDD",
  ...
  R_person <- as.person("Mael Ellison <fakeaddress@email.com> [cre]")
  aaron <-as(R_person, "creator")
